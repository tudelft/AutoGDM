...
CFilamentSimulator::CFilamentSimulator()
{
	...
	*/
	...
	SpecificGravity[0] = 1.5905;	  //ethanol   (heavier than air) -- was 1.0378
    SpecificGravity[1] = 0.5537;	  //methane   (lighter than air) -- V
	SpecificGravity[2] = 0.0696;	  //hydrogen  (lighter than air) -- V
	SpecificGravity[6] = 2.0052;	  //acetone   (heavier than air) -- was 1.4529
	...
	SpecificGravity[10] = 2.07494; 	     // isopropyl alcohol, 2-propanol, IPA
...
void CFilamentSimulator::loadNodeParameters()
{
	...
	private_nh.param<double>("filament_timeout", filament_timeout, 120);
...
void CFilamentSimulator::update_filaments_location()
{
	...
	for (int i = 0; i < current_simulation_step * numFilaments_step; i++)
	{
		if (filaments[i].valid)
		{
			if ((filaments[i].birth_time - sim_time) > filament_timeout )
			{
				filaments[i].deactivate_filament();
			}
			else
			{
...
void CFilamentSimulator::save_state_to_file()
{
	...
	// for txt saving
	std::string txt_out_filename = boost::str( boost::format("%s/iteration_%i.txt") % results_location % last_saved_step);
